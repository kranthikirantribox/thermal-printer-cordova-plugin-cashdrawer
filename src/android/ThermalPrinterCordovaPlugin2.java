...
public class ThermalPrinterCordovaPlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) {
        cordova.getThreadPool().execute(() -> {
            try {
                ...
                } else if (action.equals("requestCashDrawer")) {
                    ThermalPrinterCordovaPlugin.this.requestCashDrawer(callbackContext, args.getJSONObject(0));
                }
    ...
    public boolean printUsb() {
        ...
        if (usbConnection != null && usbManager != null) {
            ...
            // Display a custom dialog to explain the USB permission request on the main
            // thread
            cordova.getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    builder.setMessage(
                            "We need permission to access the USB device for printing. Please grant the permission.");
    ...
    private void requestBTPermissions(CallbackContext callbackContext, JSONObject data) throws JSONException {
        try {
            synchronized (this) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (!this.cordova.hasPermission(Manifest.permission.BLUETOOTH_CONNECT)) {
                        ActivityCompat.requestPermissions(this.cordova.getActivity(),
                                new String[] { Manifest.permission.BLUETOOTH_CONNECT }, PERMISSION_BLUETOOTH_CONNECT);
                    }
                    if (!this.cordova.hasPermission(Manifest.permission.BLUETOOTH_SCAN)) {
                        ActivityCompat.requestPermissions(this.cordova.getActivity(),
                                new String[] { Manifest.permission.BLUETOOTH_SCAN }, PERMISSION_BLUETOOTH_SCAN);
                    }
                } else {
                    if (!this.cordova.hasPermission(Manifest.permission.BLUETOOTH)) {
                        ActivityCompat.requestPermissions(this.cordova.getActivity(),
                                new String[] { Manifest.permission.BLUETOOTH }, PERMISSION_BLUETOOTH);
                    }
                ...
                if ((this.cordova.hasPermission(Manifest.permission.BLUETOOTH_CONNECT)
                        || this.cordova.hasPermission(Manifest.permission.BLUETOOTH_SCAN))
                        || (this.cordova.hasPermission(Manifest.permission.BLUETOOTH)
                                || this.cordova.hasPermission(Manifest.permission.BLUETOOTH_ADMIN))) {
            ...
            BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (action != null && action.equals(intentName)) {
                        ...
                        synchronized (this) {
                            UsbManager usbManager = (UsbManager) ThermalPrinterCordovaPlugin.this.cordova.getActivity()
        ...
        if (type.equals("bluetooth")) {
            ...
            try {
                ...
                for (BluetoothConnection bluetoothConnection : printerConnections.getList()) {
                    ...
                    try {
                        printerObj.put("bondState", bluetoothDevice.getBondState());
                    } catch (Exception ignored) {
                    }
                    try {
                    ...
                    try {
                        printerObj.put("type", bluetoothDevice.getType());
                    } catch (Exception ignored) {
                    }
                    try {
                        printerObj.put("features", bluetoothDevice.getUuids());
                    } catch (Exception ignored) {
                    }
                    try {
                        printerObj.put("deviceClass", bluetoothDevice.getBluetoothClass().getDeviceClass());
                    } catch (Exception ignored) {
                    }
                    try {
                        printerObj.put("majorDeviceClass", bluetoothDevice.getBluetoothClass().getMajorDeviceClass());
                    } catch (Exception ignored) {
                    }
    ...
    private DeviceConnection getDevice(CallbackContext callbackContext, String type, String id, String address,
        ...
        if (type.equals("bluetooth")) {
            ...
            // if (!this.cordova.hasPermission(Manifest.permission.BLUETOOTH)) {
            // callbackContext.error(new JSONObject(new HashMap<String, Object>() {{
            // put("error", "Missing permission for " +
            ...
            for (BluetoothConnection bluetoothConnection : printerConnections.getList()) {
                BluetoothDevice bluetoothDevice = bluetoothConnection.getDevice();
                try {
                    if (bluetoothDevice.getAddress().equals(id)) {
                        return bluetoothConnection;
                    }
                } catch (Exception ignored) {
                }
                try {
                    if (bluetoothDevice.getName().equals(id)) {
                        return bluetoothConnection;
                    }
                } catch (Exception ignored) {
                }
